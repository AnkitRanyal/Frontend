...
import { addpage, fetchProductsByFiltersAsync, selectAllProducts, selectBrands, selectCategories, selectProductListStatus, selectTotalItems } from '@/features/product/productSlice';
import { fetchAllProductAsync, fetchCarouselProductAsync, fetchBrandByFiltersAsync } from '@/features/product/productSlice';
              ...
              <Carousel responsive={responsivee} draggable={true} autoPlay={true} swipeable={true} autoPlaySpeed={3000} showDots={true} keyBoardControl={true}
                ...
                {carousel ? carousel.map((item, i) => (
                  <div className='carouselcard' key={i}>
                    <div>
                      <figure>
                        <div> <Link href={`/products/productdeatils/${item.id}`}><img src={`${item.thumbnail}`} alt='this image is missing' className='carouselimg'></img></Link></div>
                      </figure>
                    </div>
            ...
            {products.data ?
              products.data.product.map((product, i) => (
                <div key={product.id} className='card'>
                  <Link href={`/products/productdeatils/${product.id}`}> <img src={product.thumbnail} className='proimg' alt='img' /></Link>
                  {product && product.stock > 0 ? <><h4 className='producttitle'>{product.title}</h4><br></br>
                    <h5 className='productprice'>Price :&#8377; {product.price}</h5></> : <><h4 className='producttitle'>{product.title}</h4><br></br>
                    <h5 className='screenprice' style={{ color: "red", fontWeight: "600", marginLeft: "-7px" }}>Out of stock</h5></>}
                  <div className='star'>
                  ...
                  {product && product.stock > 0 ? <><button className='btn btn-info cartbtn' onClick={() => { handleadd(product.id) }}>Add to cart</button></> : <h3 className='notavalible'>Not Avalible</h3>}
                </div>
              )) : null
            }
          </div>
