...
import { selectProductById, fetchProductByIdAsync, deleteProductBYIdAsync } from "@/features/product/productSlice";
...
export default function Page(props) {
  ...
  const notifyy = () => toast.success("Item Deleted", { theme: "dark", autoClose: 2000 });
    ...
    {userlogin || loginstatus ?
      <div>
        {product && product.data && product !== null ?
          <div className="adminproductsdetails">
            ...
            <div className="adminproductinfo">
              ...
              <h5>rating : {product.data.rating}</h5>
              <h5>stock : {product.data.stock}</h5>
              <h5>brand : {product.data.brand}</h5>
              <h5>discountPercentage : {product.data.discountPercentage}</h5>
            ...
            <div className="productslinks">
              <Link href={""} className="btn btn-primary adminaddtocartbtn " onClick={() => handleadd(product.data.id)}>Add to Cart</Link><br></br><br></br>
              <Link href={`/checkout/${product.data.id}`} onClick={() => dispatch(addToBuy(product))} className="btn btn-primary adminproductsbtn ">Buy Now</Link>
              <div ><button className='btn btn-info editbtn' onClick={() => router.push(`/admin/editproduct/${product.data.id}`)}>Edit Product</button>
                <button className='btn btn-info editbtnn' onClick={() => handleremove(product.data.id)}>Delete</button></div>
            </div>
          </div>
          : null}
